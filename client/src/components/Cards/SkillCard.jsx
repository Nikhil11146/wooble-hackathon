...
export default function SkillCard({ skill, onEdit, onDelete, onVerify }) {
  ...
  return (
    <article className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-[#222d34] dark:bg-[#202c33] dark:shadow-black/25 dark:backdrop-blur">
      <div>
        <strong className="text-base text-slate-900 dark:text-[#e9edef]">{skill.name}</strong>
        {skill.category && (
          <p className="mt-1 text-sm text-slate-500 dark:text-[#8696a0]">{skill.category}</p>
        )}
        <span className={`mt-2 inline-block rounded-full px-2.5 py-1 text-xs font-semibold ${badgeClass} dark:bg-[#2a3942] dark:text-[#aebac1]`}>
          {label}
        </span>
      </div>
      <div className="flex flex-wrap gap-2">
        ...
        {onDelete && (
          <Button variant="ghost" className="min-h-10 px-3 text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/40" onClick={() => onDelete(skill)}>
            Remove
          </Button>
        )}
      </div>
